...
def migrate_policy(num_vms, vm_consumption, vm_traffic_matrix, original_placement, config, num_top_noisy_vms = 15, fixed_vms = [], cost_migration = [], steady_ratio = 0.05):
    ...
    rounds = 8
    for loop_variable in range(rounds):
        migration_to_rack = []

        link_traffic = compute_link_used_capacity(num_vms, original_placement, vm_traffic_matrix, no_vms, physical_config)
        max_traffic = max(link_traffic)
        busiest_link = link_traffic.index(max_traffic)
        print "busiest:", busiest_link

        contribution = [0 for k in range(num_vms)]
        for p in range(num_vms):
            for q in range(p+1, num_vms):
                if physical_config.which_rack[original_placement[p]] == busiest_link and physical_config.which_rack[original_placement[q]] != busiest_link or physical_config.which_rack[original_placement[p]] != busiest_link and physical_config.which_rack[original_placement[q]] == busiest_link:
                    contribution[p] += vm_traffic_matrix[p][q]
                    contribution[q] += vm_traffic_matrix[p][q]

        max_contribution = max(contribution)
        vm = contribution.index(max_contribution)

        migration_to_server = []

        # two cases: vm is in the rack linking to busiest link     or not
        if physical_config.which_rack[original_placement[vm]] != busiest_link:
            migration_to_rack.append([vm, busiest_link])
        else:
            traffic = vm_traffic_matrix[vm][:]
            traffic = sorted(traffic, reverse = True)
            for flow in traffic:
                the_other_one = vm_traffic_matrix[vm].index(flow)
                rack = physical_config.which_rack[original_placement[the_other_one]]
                if rack != busiest_link:
                    migration_to_rack.append([vm, rack])
                    break
        
        migration_to_server = choose_server_in_rack(migration_to_rack, vm_consumption, physical_config)
            
        physical_config.constraint_cpu[original_placement[vm]] += vm_consumption[vm][0]
        physical_config.constraint_memory[original_placement[vm]] += vm_consumption[vm][1]
        physical_config.constraint_disk[original_placement[vm]] += vm_consumption[vm][2]


        print migration_to_server[0], 'move', vm, 'from', physical_config.which_rack[original_placement[vm]], original_placement[vm], 'to', physical_config.which_rack[migration_to_server[0][1]], migration_to_server[0][1]

        original_placement[vm] = migration_to_server[0][1]

    # compute current traffic state of each link
    no_vms = []
    link_state = compute_link_used_capacity(num_vms, original_placement, vm_traffic_matrix, no_vms, physical_config)
    link_state = sorted(link_state, reverse = True)
    print "traffic on each link: ", link_state
